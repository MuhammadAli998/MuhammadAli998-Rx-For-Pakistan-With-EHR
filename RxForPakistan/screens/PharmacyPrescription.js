...
const Pharmacy = ({ navigation }) => {
  ...
  const { Cnic, Pharmacydb,highbp,lowbp,temperature,sugar,ccomplaint} = route.params;
console.log(Cnic);
console.log(Pharmacydb);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://${ip}/PharmacyApi/api/Pharmacy/Getmedicine?Pharmacydb=${Pharmacydb}&did=${Cnic}`);
    ...
    <ScrollView style={styles.container}>
      ...
      <DataTable>
          <DataTable.Header style={styles.tableHeader}>
            ...
            <DataTable.Title>High BP</DataTable.Title>
            <DataTable.Title>Low BP</DataTable.Title>
            <DataTable.Title>Temperature</DataTable.Title>
            <DataTable.Title>Sugar</DataTable.Title>
            
          </DataTable.Header>
          
            <DataTable.Row >
              <DataTable.Cell>1</DataTable.Cell>
              <DataTable.Title>{ccomplaint}</DataTable.Title>
            <DataTable.Title>{highbp}</DataTable.Title>
            <DataTable.Title>{lowbp}</DataTable.Title>
            <DataTable.Title>{temperature}</DataTable.Title>
            <DataTable.Title>{sugar}</DataTable.Title>
            </DataTable.Row>
