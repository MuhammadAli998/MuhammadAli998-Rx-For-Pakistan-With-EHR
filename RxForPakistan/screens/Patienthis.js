...
const Pharmacy = ({navigation}) => {
  ...
  const gethistory = ()  => {
    const apiUrl =`http://${ip}/DoctorApi/api/Doctor/GetPatient?doctordb=${doctordb}&cnic=${cnic}`
    ;
      ...
      <View style={styles.flex2}>
          <DataTable>
            ...
            {historyData.map((historyItem, index) => (
              <TouchableOpacity
                key={index}
                onPress={() => navigation.navigate('DrPatientMedicine', { Aid: historyItem.Aid,doctordb: doctordb })}
