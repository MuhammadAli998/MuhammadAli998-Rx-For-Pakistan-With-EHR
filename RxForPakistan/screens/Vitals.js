...
const Appointment = ({ navigation }) => {
  ...
  const addVitals = async () => {
    try {
      const response = await fetch(`http://${ip}/DoctorApi/api/Doctor/AddVitals?doctordb=${doctordb}&appointmentId=${Aid}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        Alert.alert('Successfully Added', responseData.message);
        setHighBP('');
        setLowBP('');
        setTemperature('');
        setSugar('');
        ...
        const responseData = await response.json();
        console.log(responseData);
        Alert.alert('Already Added for this Appointment', responseData.message);
      } else {
        const responseData = await response.json();
        console.log(responseData);
...
<TextInput
        ...
        placeholder="Low BP(mmhg)"
        keyboardType="Text]"
      />
</View>
<View   style={styles.VitalView}>
