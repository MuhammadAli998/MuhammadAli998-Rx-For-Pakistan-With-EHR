...
const Signup = ({navigation}) => {
    ...
    const banMedicine = () => {
      ...
      const data = {
        ...
        MedicineName2: Medicine,
        Comment: comment
      };
      
      if (!selectedMedicine || !Medicine) {
        Alert.alert('Error', 'Please select two medicines to add a contraindication.');
      } else {
        // Make the HTTP POST request
        fetch(`http://${ip}/MedicineApi/api/Medicine/AddMedicineContraindication`, {
          method: 'POST',
          headers: {
          ...
          .then(response => {
            if (response.status === 200) {
              ...
              Alert.alert( 'Medicine to Medicine contraindication added Successfully')
                
            } else if (response.status === 400) {
              // Bad request, show an alert message with the error text
              response.text().then(errorMessage => {
                Alert.alert('Error', errorMessage);
              });
            } else if (response.status === 500) {
              // Internal server error, show an alert message with the error text
              response.text().then(errorMessage => {
                Alert.alert('Error', errorMessage);
              });
            } else {
              // Unknown error, show a generic error message
...
<View style={styles.flex1}>
      ...
      <Text style={styles.signup}>Add Medicinesc To Medicine Contraindications </Text>
      <View>
        ...
        />
         <Text style={styles.Text9}>Contradicited to: </Text>
        <SelectList
          setSelected={(val) => setMedicine(val)}
          data={medicineList}
          save="value"
          label="Medicine"
          placeholder="Medicine"
